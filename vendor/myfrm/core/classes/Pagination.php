...
class Pagination
{
    ...
    
    {
        ...
        $this->current_page = $this->getCurrrentPage();
    ...
    public function getCurrrentPage(): int
    {
        if ($this->page < 1) {
            $this->page = 1;
        }
    ...
    public function getHtml(): string
    {
        ...
        for ($i = 1; $i <= $this->mid_size; $i++) {
            if ($this->current_page + $i <= $this->count_pages) {
                $pages_right .= "<li class='page-item'><a href='". $this->getLink($this->current_page + $i) ."' class='page-link'>" .
        ...
        for ($i = $this->mid_size; $i > 0; $i--) {
            if ($this->current_page - $i > 0) {
                $pages_left .= "<li class='page-item'><a href='". $this->getLink($this->current_page - $i) ."' class='page-link'>" .
                ($this->current_page - $i) . "</a></li>";
            }
        }

        return "<nav aria-label='Page navigation example'><ul class='pagination'>" . $start_page . $back . $pages_left . 
        "<li class='page-item active'><a class='page-link'>" . $this->current_page . "</a></li>" . 
